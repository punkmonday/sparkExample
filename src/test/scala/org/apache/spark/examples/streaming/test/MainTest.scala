...
import org.apache.spark.examples.streaming.utils.StructTypeConverter
...
object MainTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    df1.columns.foreach(fieldSet => {
      df2 = df2.withColumn(fieldSet, addCol(df2.col(fieldSet)))
    })
    ...
    df2.writeStream
      ...
      ////          value.schema.fieldNames.foreach(field => {
      ////            println(CommonFilter.filter(value.getAs(field).toString, map))
      ////          })
      //
      //        }
      //
